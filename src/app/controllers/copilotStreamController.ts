...
export default class CopilotStreamController {
  ...
  private setMessage: ((message: string) => void) | undefined; // the current answer element to be updated from socket events
  ...
  public async askQGPT({
    ...
    query: string;
    // setChatHistory: (prev: ({ role: string; message: string; })[]) => ({ role: string; message: string; }[]);
